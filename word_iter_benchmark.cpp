...
int main()
{
	const std::string input = R"(To Sherlock Holmes she is always _the_ woman. I have seldom heard him
...
and was shown up to the chamber which had formerly been in part my own.)";

	{
		...
		auto it = std::sregex_token_iterator(input.begin(), input.end(), re, -1);
		...
		std::cout << "regex duration:     " << duration_cast<microseconds>(t2 - t1).count() << "us (" << count << ")\n";
	}

	{
		...
		const auto t2 = high_resolution_clock::now();
		std::cout << "WordIter duration: " << duration_cast<microseconds>(t2 - t1).count() << "us (" << count << ")\n";
	}

	{
		...
		const auto t2 = high_resolution_clock::now();
		std::cout << "boost duration:   " << duration_cast<microseconds>(t2 - t1).count() << "us (" << count << ")\n";
	}
}
